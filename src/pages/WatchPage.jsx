...
const WatchPage = () => {
  ...
  if (!episodes || !anime) return <Loader className="h-screen" />;
    ...
    <div className="bg-backGround pt-16 max-w-screen-2xl mx-auto px-3 md:px-6 pb-6">
      ...
      <div className="flex flex-col-reverse lg:flex-row gap-5">
        ...
        <div className="flex-1 flex flex-col lg:flex-row gap-5">
          ...
          {/* Anime Info */}
          <div className="bg-[#1a1a1f] rounded-xl p-4 text-gray-300 w-full lg:w-[30%] flex flex-col max-h-[70vh]">
            <div className="flex flex-col sm:flex-row lg:flex-col gap-3">
              <img
                src={anime?.poster}
                alt={anime?.title}
                className="w-40 h-auto rounded-lg object-cover mx-auto"
              />
              <div className="flex flex-col">
                <h2 className="text-white font-semibold text-lg mb-2">
                  {anime?.title}
                </h2>
                <p className="text-sm text-gray-400 mb-3 overflow-y-auto">
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  {anime?.genres?.map((g) => (
                    <span
                      key={g}
                      className="bg-primary/20 text-primary px-2 py-1 rounded-md"
                    >
                      {g}
                    </span>
                  ))}
      ...
      {/* Recommended Section */}
      <div className="mt-10">
        <h3 className="text-white text-lg font-semibold mb-4">Recommended</h3>
        <Recommended id={id} />
      </div>
    </div>
  );
