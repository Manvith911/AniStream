        ...
        <div className="flex flex-col px-4 sm:px-6 md:px-10">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="relative w-full sm:ml-6 sm:max-w-[400px]">
              <form onSubmit={handleSubmit} className="flex items-center gap-2 bg-[#FBF8EF] px-3 py-1 rounded-md w-full">
                <input
                  value={value}
                ...
                <div className="absolute top-full mt-1 left-0 w-full max-w-full bg-card z-50 rounded-md overflow-hidden shadow-lg">
                  {isLoading ? <Loader /> :
                    data && data?.data.length ? (
                      <>
                        {data?.data?.map(item => (
                          <div key={item.id} onClick={() => navigateToAnimePage(item.id)}
                            ...
                            <div className="poster shrink-0 relative w-10 h-14">
                              <img src={item.poster} alt={item.title} className="h-full w-full object-cover object-center rounded-sm" />
                            </div>
                            <div className="info">
                        ...
                        <button className="py-2 flex justify-center items-center gap-2 bg-primary text-black w-full" onClick={handleSubmit}>
            ...
            <div className="flex justify-end sm:ml-4">
              {profile ? (
                <div className="flex items-center gap-2">
                  <img src={profile.avatar_url || "/default-avatar.png"} className="w-8 h-8 rounded-full" />
                  <span>{profile.username || profile.email}</span>
                  <button onClick={logout} className="ml-2 text-sm">Logout</button>
                </div>
              ) : (
                <button onClick={goToLogin} className="bg-primary text-black px-4 py-1 rounded-md text-sm font-semibold hover:bg-yellow-400 transition">
                  Login
                </button>
              )}
            </div>
